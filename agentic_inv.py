import numpy as np
class AgenticInvSystem:
    def __init__(s, n=100, rate=50):
        s.n = n; s.rate = rate
    def simulate(s, periods=365, seed=42):
        rng = np.random.default_rng(seed)
        demand = np.maximum(rng.normal(200, 50, periods), 0)
        results = []; inv = 200*2; cost = 0
        for t in range(periods):
            d = demand[t]
            order = max(0, d*1.1 - inv + 200)
            inv = max(0, inv + order - d)
            holding = inv * 0.05
            shortage = max(0, d - inv) * 25
        ...
        return {{"total_cost": round(cost,0), "avg_inventory": round(np.mean([r["inventory"] for r in results]),1),
                ...
                "periods": len(results)}}
if __name__=="__main__": print(AgenticInvSystem().simulate())
